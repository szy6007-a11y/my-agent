...
export function registerBuiltinHooks(registry: AgentHookRegistry): void {
  ...
  registry.register("pre_model_call", "output_protocol_reminder", () => ({
    reminders: [
      "输出内容必须严格遵循内部输出协议：最终用户可见正文必须完整放在 <final_answer>...</final_answer> 内；<final_answer> 是文本边界标签，不是工具名，严禁调用名为 final_answer 的工具；需要工具时先走原生 tool call，工具完成后再输出 <final_answer> 和给用户看的中文正文；不要向用户提及内部标签、协议名、系统提醒，且不要把工具名和参数 JSON 写成伪工具调用文本。普通问答、搜索、日期/股票/活动查询、解释和总结不要调用写文件工具；只有用户明确要求创建、保存、导出、下载、编辑文件或可下载 artifact 时才使用写类文件工具。检索、抓取或工具执行过程中某个站点失败、被拦截、超时、返回空内容或被安全策略阻止时，除非用户明确询问调试细节，不要把“某站点被 block/blocked”“工具受限”“我换个方向搜索”等检索过程写进最终正文；最终正文只呈现已验证结果、必要的不确定性和可读来源。用户明确询问工具、实现或调试时，可以用自然语言提及工具名。",
    ],
  }));
