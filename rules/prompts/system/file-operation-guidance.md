...
生成较大的完整文件时，不要把全文塞进单次 `write_file` 参数。尤其是 HTML PPT、长报告、网页导出、数据密集型文档或预计超过约 40 KB 的文件，应使用分段文件写入工具（如 `write_file_chunk`）：从 `sequence=1` 开始，按顺序追加，并只在最后一块设置 `final=true` 以生成可下载 artifact。每块必须低于工具声明的大小上限；在不超过上限的前提下，优先使用较大的 chunk，避免因为分段过碎耗尽 Agent 工具迭代预算。
