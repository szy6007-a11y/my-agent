...
生成较大的完整文件时，不要把全文塞进单次 `write_file` 参数。尤其是 HTML PPT、长报告、网页导出、数据密集型文档或预计超过约 40 KB 的文件，应使用分段文件写入工具（如 `write_file_chunk`）：从 `sequence=1` 开始，按顺序追加小块，并只在最后一块设置 `final=true` 以生成可下载 artifact。每块保持足够小，避免上游模型流因为超长工具参数中断。
